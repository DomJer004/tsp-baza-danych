...
# =========================================================
    ...
    # --- 1. GŁÓWNY WIDOK ---
        ...
        # Linki do analizy rywala (ID meczu - pomocnicze)
        if df_m is not None:
            def create_link(row):
                try:
        ...
        with tab1:
            if df_det_sq is not None:
                ...
                # Grupujemy, aby mieć unikalne mecze
                ...
                # Funkcja dodająca emotki do selectboxa
                ...
                # --- DEFINICJA STARYCH SEZONÓW (Bez minut zmian) ---
        ...
        # ====================================================================
        # TAB 2: ANALIZA RYWALA (ULEPSZONA)
        # ====================================================================
        with tab2:
            st.subheader("🆚 Analiza Rywala i Historia Spotkań")

            if df_m is not None:
                # 1. Pobranie listy rywali
                rivs = sorted(df_m['rywal'].dropna().astype(str).unique()) if 'rywal' in df_m.columns else []
                sel_r = st.selectbox("Wybierz rywala do analizy:", [""] + rivs)

                if sel_r:
                    # Filtrujemy mecze z tym rywalem
                    col_date_m = next((c for c in df_m.columns if c in ['data meczu', 'data', 'dt_obj']), None)

                    rival_matches = df_m[df_m['rywal'] == sel_r].copy()

                    # Konwersja daty
                    if col_date_m:
                        rival_matches['DT_Match'] = pd.to_datetime(rival_matches[col_date_m], dayfirst=True,
                                                                   errors='coerce')
                        rival_matches = rival_matches.sort_values('DT_Match', ascending=False)

                    # --- A. OBLICZANIE STATYSTYK ---
                    ...
                    # --- B. WIZUALIZACJA ---
                    st.markdown(f"### 📊 Bilans vs {sel_r}")
                    k1, k2, k3, k4 = st.columns(4)
                    k1.metric("Mecze", total_matches)
                    k2.metric("Zwycięstwa", wins,
                              delta=f"{(wins / total_matches * 100):.0f}%" if total_matches else None)
                    k3.metric("Remisy / Porażki", f"{draws} / {losses}")
                    k4.metric("Bramki", f"{gf} : {ga}", delta=f"{gf - ga}")

                    st.divider()

                    # --- C. TABELA HISTORII ---
                    st.markdown("#### 📜 Historia Spotkań")
                    st.caption("Kliknij w wiersz, aby załadować szczegóły składu w zakładce 'Raporty Meczowe'.")

                    rival_matches['Data'] = rival_matches['DT_Match'].dt.strftime('%d.%m.%Y')
                    rival_matches['Gdzie'] = rival_matches['dom'].apply(
                        lambda x: "🏠 Dom" if str(x).lower() in ['1', 'true', 'dom', 'tak'] else "🚌 Wyjazd")

                    cols_final = ['Data', 'sezon', 'Gdzie', 'wynik']

                    event = st.dataframe(
                        rival_matches[cols_final],
                        use_container_width=True,
                        hide_index=True,
                        on_select="rerun",
                        selection_mode="single-row",
                        column_config={"wynik": st.column_config.TextColumn("Wynik", width="small")}
                    )

                    # --- D. LOGIKA KLIKNIĘCIA (NAPRAWA LINKÓW) ---
                    if event.selection.rows:
                        idx = event.selection.rows[0]
                        selected_date = rival_matches.iloc[idx]['DT_Match'].date()

                        # Szukamy meczu po dacie w wystepy.csv
                        if df_det_sq is not None and 'Data_Sort' in df_det_sq.columns:
                            found_match = df_det_sq[df_det_sq['Data_Sort'].dt.date == selected_date]

                            if not found_match.empty:
                                match_record = found_match.iloc[0]
                                found_label = match_record['Mecz_Label']
                                found_season = match_record['Sezon']

                                st.session_state['cm_season_sel'] = found_season
                                st.session_state['cm_match_sel'] = found_label

                                st.success(
                                    f"✅ Załadowano mecz z dnia {selected_date}! Przejdź do zakładki '📝 Raporty Meczowe'.")
                                time.sleep(1.5)
                                st.rerun()
                            else:
                                st.warning(f"⚠️ Nie znaleziono składu dla meczu z {selected_date}.")
                        else:
                            st.error("Błąd danych szczegółowych.")
            else:
                st.error("Brak pliku mecze.csv")

        # ====================================================================
        # TAB 3: STATYSTYKI (Z IKONAMI W SERIACH)
        # ====================================================================
        with tab3:
            ...
            if df_m is not None:
                ...
                if "Wyjazd" not in f_mode:
                    ...
                    if col_att:
                        ...
                        df_att = df_att[df_att['Widzów_Num'] > 0]
                        ...
                        if not df_att.empty:
                            ...
                            if 'dt_obj' in df_att.columns:
                                ...
                                if HAS_PLOTLY:
                                    fig = go.Figure(data=[go.Bar(x=monthly_att['Miesiąc'], y=monthly_att['Średnia'],
                                                                 text=monthly_att['Średnia'].astype(int),
                                                                 textposition='auto', marker_color='#28a745')])
                ...
                with st.expander("📜 Pokaż mecze z serii"):
                    ...
                    # Funkcja wyświetlająca tabelę serii z ikonami
                    def show_streak_with_icons(lst):
                        if lst:
                            df_streak = pd.DataFrame(lst)
                            # Dodanie ikon do kolumny dom/wyjazd
                            df_streak['Gdzie'] = df_streak['dom'].apply(
                                lambda x: "🏠 Dom" if str(x).lower() in ['1', 'true', 'dom', 'tak'] else "🚌 Wyjazd")

                            st.dataframe(
                                df_streak[['data meczu', 'rywal', 'wynik', 'Gdzie']],
                                hide_index=True,
                                use_container_width=True
                            )
                        else:
                    ...
                    with ts1:
                        show_streak_with_icons(s_win)
                    with ts2:
                        show_streak_with_icons(s_no_loss)
                    with ts3:
                        show_streak_with_icons(s_loss)
            else:
                st.error("Brak pliku mecze.csv")
    ...
    if df_p is None or df_w is None:
        ...
        # --- LEGENDA ODZNAK (NOWOŚĆ) ---
        with st.expander("ℹ️ Legenda Odznak (Co oznaczają ikony?)", expanded=False):
            st.markdown("""
            Odznaki są przyznawane automatycznie na podstawie statystyk zgromadzonych w bazie danych:

            | Ikona | Nazwa Odznaki | Warunek otrzymania |
            | :---: | :--- | :--- |
            | 💯 | **Klub 100** | Zawodnik rozegrał co najmniej **100 oficjalnych meczów** w TSP. |
            | 👑 | **Legenda (Strzelec)** | Zawodnik znajduje się w **TOP 10** najlepszych strzelców w historii. |
            | 🚀 | **Awans** | Zawodnik grał w sezonie zakończonym awansem do Ekstraklasy (**2011**, **2020**). |
            | 🎩 | **Hat-trick Hero** | Zdobył przynajmniej jednego **hat-tricka** (3 gole w jednym meczu). |
            | 🃏 | **Super Joker** | Zdobył co najmniej **3 gole** wchodząc z ławki rezerwowych. |
            | 👶 | **Młody Wilk** | Debiutował w wieku, który plasuje go w **TOP 10 najmłodszych** debiutantów. |
            | 👴 | **Nestor** | Wystąpił w wieku, który plasuje go w **TOP 10 najstarszych** zawodników. |
            | 🟥 | **Bad Boy** | Otrzymał w karierze co najmniej **2 czerwone kartki**. |
            | 🫁 | **Żelazne Płuca** | Spędził na boisku łącznie ponad **5000 minut**. |
            | 🦅 | **Weteran** | Rozegrał w barwach klubu co najmniej **5 sezonów**. |
            """)

        st.divider()

        # --- PRZYGOTOWANIE DANYCH DO WIEKU ---
        # Łączymy występy z datami urodzenia
        df_p_dates = df_p[['imię i nazwisko', 'data urodzenia', 'Narodowość']].drop_duplicates(
        ...
        df_p_dates = prepare_flags(df_p_dates, 'Narodowość')  # Dodajemy flagi
        ...
        if 'Data_Sort' in df_w.columns and 'Zawodnik_Clean' in df_w.columns:
            ...
            def format_age_display(days):
                ...
                rest_days = int(days % 365.25)
                return f"{years} lat, {rest_days} dni"
        ...
        with tab1:
            ...
            # Doczepiamy flagi
            col_match_agg = pd.merge(col_match_agg, df_p_dates[['imię i nazwisko', 'Flaga']],
                                     left_on='Zawodnik_Clean', right_on='imię i nazwisko', how='left')

            top_matches = col_match_agg.sort_values('Liczba_Meczów', ascending=False).head(15)
            top_matches = get_medals(top_matches)

            st.dataframe(
                top_matches[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Liczba_Meczów']],
                hide_index=True, use_container_width=True,
                column_config={
                    "Flaga": st.column_config.ImageColumn("", width="small"),
                    "Liczba_Meczów": st.column_config.ProgressColumn("Mecze", format="%d", min_value=0,
                                                                     max_value=int(top_matches['Liczba_Meczów'].max()))
                }
            )
        ...
        with tab2:
            if not df_age.empty:
                ...
                with c_young:
                    ...
                    st.dataframe(
                        youngest[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt', 'Data_Sort']],
                        hide_index=True, use_container_width=True,
                        column_config={
                            "Flaga": st.column_config.ImageColumn("", width="small"),
                ...
                with c_old:
                    ...
                    st.dataframe(
                        oldest[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt', 'Data_Sort']],
                        hide_index=True, use_container_width=True,
                        column_config={
                            "Flaga": st.column_config.ImageColumn("", width="small"),
                            "Data_Sort": st.column_config.DateColumn("Data Meczu", format="DD.MM.YYYY"),
                            "Wiek_Txt": st.column_config.TextColumn("Wiek")
                        }
                    )
            else:
        ...
        with tab3:
            ...
            scorers = pd.merge(scorers, df_p_dates[['imię i nazwisko', 'Flaga']],
                               left_on='Zawodnik_Clean', right_on='imię i nazwisko', how='left')
            scorers = get_medals(scorers)

            st.dataframe(
                scorers[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Gole_Num']],
                hide_index=True, use_container_width=True,
                column_config={
                    "Flaga": st.column_config.ImageColumn("", width="small"),
                    "Gole_Num": st.column_config.NumberColumn("Liczba Goli", format="%d ⚽")
                }
            )
            ...
            if not df_age.empty:
                ...
                if not df_goals.empty:
                    ...
                    with c_ys:
                        ...
                        st.dataframe(y_sc[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt']], hide_index=True,
                                     use_container_width=True,
                    ...
                    with c_os:
                        ...
                        st.dataframe(o_sc[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt']], hide_index=True,
                                     use_container_width=True,
                                     column_config={"Flaga": st.column_config.ImageColumn("", width="small")})
        ...
        with tab4:
            ...
            with c_hat:
                ...
                if not hats.empty:
                    hats = hats.sort_values('Data_Sort', ascending=False)  # Najnowsze na górze
                    st.dataframe(
            ...
            with c_res:
                ...
                if df_m is not None and 'wynik' in df_m.columns:
                    ...
                    st.dataframe(wins[['Miejsce', 'sezon', 'rywal', 'wynik']], hide_index=True,
                                 use_container_width=True)
        ...
        with tab5:
            ...
            if 'Czerwone' in df_w.columns:
                ...
                # Dodanie flag
                reds = pd.merge(reds, df_p_dates[['imię i nazwisko', 'Flaga']], left_on='Zawodnik_Clean',
