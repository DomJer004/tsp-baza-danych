...
def render_player_profile(player_name):
    ...
    # Sortowanie i wybór unikalnego wiersza profilowego
    ...
    # --- B. ODZNAKI (POPRAWIONE) ---
    # Przekazujemy załadowane dane (df_w=występy, df_p=profil), aby funkcja odznak ich nie szukała ponownie
    ...
    # --- G. HISTORIA SZCZEGÓŁOWA ---
    st.markdown("---")
    st.subheader("📜 Szczegółowa historia meczowa")
    if not p_hist.empty:
        if 'Data_Sort' in p_hist.columns: p_hist = p_hist.sort_values('Data_Sort', ascending=False)
        pos_str = str(row.get('pozycja', '')).lower().strip()
        is_gk = ('bramkarz' in pos_str or 'gk' in pos_str)

        # Logika dla bramkarzy (Clean Sheets / Wpuszczone)
        if is_gk:
            def analyze_gk(r):
                conceded = 0
                icon = ""
                ...
                # Min. 45 minut na boisku i 0 wpuszczonych = czyste konto
        ...
        st.dataframe(p_hist[final_cols].reset_index(drop=True), use_container_width=True, hide_index=True,
                     column_config={
        ...
        # Szukamy innych piłkarzy w tych samych meczach
        mates = df_det_goals[df_det_goals['Mecz_Label'].isin(my_m)]
        mates = mates[mates['Zawodnik_Clean'] != player_name]

        if not mates.empty:
            tm = mates['Zawodnik_Clean'].value_counts().head(10).reset_index()
            tm.columns = ['Kolega z zespołu', 'Wspólne Mecze']

            # Dodajemy flagi kolegów jeśli dostępne
            if 'Flaga' in df_uv_sorted.columns:
                f_map = df_uv_sorted[['imię i nazwisko', 'Flaga']].drop_duplicates('imię i nazwisko')
                tm = pd.merge(tm, f_map, left_on='Kolega z zespołu', right_on='imię i nazwisko', how='left')
            else:
                tm['Flaga'] = None

            tm.index += 1
            tm.insert(0, 'Miejsce', tm.index.map(
                lambda x: f"🥇 1." if x == 1 else (f"🥈 2." if x == 2 else (f"🥉 3." if x == 3 else f"{x}."))))

            c_tm1, c_tm2 = st.columns([2, 1])
            with c_tm1:
                st.dataframe(tm[['Miejsce', 'Flaga', 'Kolega z zespołu', 'Wspólne Mecze']], use_container_width=True,
                             hide_index=True,
                             column_config={"Flaga": st.column_config.ImageColumn("", width="small"),
                                            "Wspólne Mecze": st.column_config.ProgressColumn("Mecze", format="%d",
                                                                                             min_value=0, max_value=int(
                                                    tm['Wspólne Mecze'].max()))})
        else:
            st.info("Brak danych o kolegach z zespołu.")
    else:
        st.info("Brak historii meczowej.")


def render_coach_profile(coach_name):
    """Generuje pełny profil trenera obsługujący WIELE KADENCJI + MULTI-FLAGI."""
    ...
    # Normalizacja flag i nazw kolumn
    ...
    with c1:
        # [POPRAWKA] Używamy get_multi_flags_html zamiast st.image
        flags_html = get_multi_flags_html(nat_raw)
        if flags_html:
            st.markdown(flags_html, unsafe_allow_html=True)
        else:
...
def get_player_record_badges(player_name, df_w=None, df_p=None):
    ...
    try:
        ...
        if p_data.empty and df_p is None: return []

        row_p = pd.DataFrame()
        if df_p is not None and 'imię i nazwisko' in df_p.columns:
            row_p = df_p[df_p['imię i nazwisko'] == player_name]

        position = str(row_p.iloc[0]['pozycja']).lower() if not row_p.empty else ""
        nation = str(row_p.iloc[0]['Narodowość']).lower() if not row_p.empty else "polska"
        ...
        if not row_p.empty:
            for col in ['suma', 'mecze', 'liczba']:
                if col in row_p.columns:
                    val = pd.to_numeric(row_p.iloc[0][col], errors='coerce')
                    if pd.notna(val): manual_matches = int(val)
                    break
        ...
        if total_matches >= 100:
            badges.append({"icon": "💯", "text": f"Klub 100 ({total_matches} spotkań)", "color": "#d63031"})

        # 🚀 AWANS
        if not p_data.empty and 'Sezon' in p_data.columns:
            seasons_played = set(p_data['Sezon'].unique())
            promotions = []
            if any(x in seasons_played for x in ['2010/2011', '2010/11']): promotions.append("2011")
            if any(x in seasons_played for x in ['2019/2020', '2019/20']): promotions.append("2020")
            if promotions:
                badges.append({"icon": "🚀", "text": f"Awans ({', '.join(promotions)})", "color": "#FFD700"})

        # --- ODZNAKI SPECJALNE ---
        if not p_data.empty:
            # 🧱 MURARZ
            if 'bramkarz' in position or 'gk' in position:
                clean_sheets = 0
                    ...
                    try:
                        ...
                        if mins >= 45:
                            ...
                            if len(parts) >= 2:
                                g1, g2 = int(parts[0]), int(parts[1])
                                role = str(r.get('Rola', '')).lower()
                                conceded = g2 if ('gospodarz' in role or 'dom' in role or 'u siebie' in role) else g1
                                if conceded == 0: clean_sheets += 1
                    except:
                        pass
                if clean_sheets >= 20:
                    badges.append(
                        {"icon": "🧱", "text": f"Wielki Mur ({clean_sheets} czystych kont)", "color": "#2c3e50"})
                elif clean_sheets >= 10:
                    badges.append(
                        {"icon": "🧤", "text": f"Pewny Punkt ({clean_sheets} czystych kont)", "color": "#34495e"})

            # ⚖️ DŻENTELMEN & 🟥 BAD BOY
            reds_total = pd.to_numeric(p_data['Czerwone'], errors='coerce').fillna(0).sum()
            if total_matches >= 30 and reds_total == 0:
                badges.append({"icon": "⚖️", "text": "Dżentelmen Boiska (Fair Play)", "color": "#16a085"})
            if reds_total >= 2:
                badges.append({"icon": "🟥", "text": f"Bad Boy ({int(reds_total)} cz. kartki)", "color": "#e74c3c"})

            # 🔄 ZADANIOWIEC
            subs_in = len(p_data[p_data['Status'] == 'Wszedł'])
            if subs_in >= 20:
                badges.append({"icon": "🔄", "text": f"Zadaniowiec ({subs_in} wejść)", "color": "#8e44ad"})

            # 🎩 HAT-TRICK & 🃏 JOKER
            p_data['Gole_Num'] = pd.to_numeric(p_data['Gole'], errors='coerce').fillna(0)
            hattricks = len(p_data[p_data['Gole_Num'] >= 3])
            if hattricks > 0:
                badges.append({"icon": "🎩", "text": f"Hat-trick Hero ({hattricks}x)", "color": "#2ecc71"})

            bench_goals = p_data[p_data['Status'] == 'Wszedł']['Gole_Num'].sum()
            if bench_goals >= 3:
        ...
        # 🌍 ZAGRANICZNY FILAR (POPRAWIONE)
        # Warunek: Musi mieć narodowość, NIE może mieć w niej słowa "polska", "poland", "pl"
        has_pl = any(x in nation for x in ['polska', 'poland', 'pl'])
        valid_nation = nation not in ['-', '', 'nan']

        if valid_nation and not has_pl and total_matches >= 50:
            badges.append({"icon": "🌍", "text": f"Zagraniczny Filar ({total_matches} meczów)", "color": "#2980b9"})
...
# =========================================================
    ...
    # --- 1. WIDOK GŁÓWNY ---
        ...
        if df_det is None or df_bio is None:
            ...
            # --- OBLICZANIE RUCHÓW KADROWYCH ---
            # Domyślne puste listy
            arrived_list = []
            left_list = []
            transfer_text_in = "-"
            transfer_text_out = "-"
            ...
            try:
                ...
                if found_prev:
                    ...
                    # Wyliczamy listy
                    arrived_list = sorted(list(curr_players - prev_players))
                    left_list = sorted(list(prev_players - curr_players))

                    # Teksty do boksu statystyk biorą długość tych list
            ...
            # --- STATYSTYKI ZBIORCZE (TU WSTAWIAMY LICZBY Z LIST) ---
            if not show_only_youth:
                ...
                <div style="background-color: var(--secondary-background-color); padding: 15px; border-radius: 10px; border: 1px solid #ccc; margin-bottom: 20px;">
                    <div style="display: flex; justify-content: space-around; flex-wrap: wrap;">
                        ...
                        <div style="text-align: center; margin: 5px; border-left: 1px solid #666; padding-left: 15px;">
                            <small style="color: gray;">Ruchy kadrowe</small><br>
                ...
                if arrived_list or left_list:
                    with st.expander("🔄 Zobacz szczegóły transferów (Przyszli / Odeszli)", expanded=False):
                        c_in, c_out = st.columns(2)
                        with c_in:
                            st.markdown(f"**📥 Przyszli ({len(arrived_list)})**")
                            # Wyświetlamy jako listę punktowaną lub po przecinku
    ...
    # --- 1. GŁÓWNY WIDOK ---
        ...
        # Helpery renderowania raportu
        def render_match_report_logic(match_label, squad_df):
            if squad_df.empty:
                st.warning("Brak składu w bazie.")
                return
            try:
            ...
            map_in_to_out = {}
            map_out_to_in = {}
            condition_in = (squad_df['Status'] == 'Wszedł') | (
                        (squad_df['Status'] == 'Czerwona kartka') & (squad_df['Wejście'] > 0))
            in_rows = squad_df[condition_in].sort_values('Minuta_Zmiany_Real')
            out_rows = squad_df[squad_df['Status'].isin(['Zszedł'])].sort_values('Minuta_Zmiany_Real')
            used_out = []
            ...
            def render_row(row, is_sub=False):
                c1, c2, c3 = st.columns([1, 4, 3])
                mins = int(row.get('Minuty', 0))
                entry = int(row.get('Minuta_Zmiany_Real', 0))
                ...
                with c1:
                    if is_sub:
                        ...
                        st.write("90'" if (mins == 0 and status in ['Cały mecz', 'Grał']) else (
                            f"{mins}'" if mins > 0 else "-"))
                with c2:
                    if st.button(name,
                                 key=f"btn_{match_label}_{name}_{is_sub}_{st.session_state.get('uploader_key', 0)}",
                                 use_container_width=True):
                ...
                if is_sub:
                    ...
                    evs.append(f"<span style='color:#28a745; font-size:0.85em;'>⬆️ {txt}</span>")
                elif status == 'Zszedł':
                    rep = map_out_to_in.get(name)
                    min_info = f" ({entry}')" if entry > 0 else ""
                    txt = f"zm: {rep}{min_info}" if rep else "Zejście"
                    evs.append(f"<span style='color:#dc3545; font-size:0.85em;'>⬇️ {txt}</span>")
                with c3:
                    if evs: st.markdown(" ".join(evs), unsafe_allow_html=True)

            starters = squad_df[(squad_df['Status'].isin(['Cały mecz', 'Zszedł', 'Grał', 'Czerwona kartka'])) & (
                        squad_df['Wejście'] == 0)].sort_values('File_Order')
            subs = squad_df[(squad_df['Status'] == 'Wszedł') | (
                        (squad_df['Status'] == 'Czerwona kartka') & (squad_df['Wejście'] > 0))].sort_values(
                'Minuta_Zmiany_Real')
            unused = squad_df[squad_df['Status'] == 'Rezerwowy']

            k_l, k_r = st.columns(2)
            with k_l:
                st.subheader("🏟️ Wyjściowa XI")
                if not starters.empty:
    ...
    # [1] GŁÓWNY WIDOK
        ...
        if df_p is None or df_w is None:
            ...
            # 1. Konwersja na string i czyszczenie
            ...
            sub_g = df_w[df_w['Status'] == 'Wszedł'].groupby('join_key')['Gole_Num'].sum()
            list_joker = df_w[df_w['join_key'].isin(sub_g[sub_g >= 3].index)]['Zawodnik_Clean'].unique().tolist()
            ...
            with tab_badges:
                ...
                with c2:
                    ...
                    show_badge_group("Super Joker", list_joker, "🃏", "Rezerwowi z min. 3 golami po wejściu.")
                with c3:
                    show_badge_group("Dżentelmen", list_gentleman, "⚖️", "Min. 30 meczów bez czerwonej kartki.")
