...
def render_player_profile(player_name):
    ...
    # --- NOWOŚĆ: ODZNAKI / REKORDY ---
    badges = get_player_record_badges(player_name)
    if badges:
        st.success("🏆 **Osiągnięcia Klubowe:** " + " | ".join(badges))
...
# =========================================================
    ...
    # Zmieniono liczbę zakładek na 3 (usunięto Transfery i ME)
    tab1, tab2, tab3 = st.tabs(["Baza Zawodników", "Strzelcy", "Klub 100"])

    # =========================================================
    # TAB 1: BAZA ZAWODNIKÓW (PEŁNA LISTA + PROFIL)
    # =========================================================
    with tab1:
        ...
        if df_uv is not None:
            ...
            if df_det is not None:
                if 'Zawodnik_Clean' in df_det.columns and 'Minuty' in df_det.columns:
            ...
            # 2. Ustalanie kolumny z meczami w pliku bazowym (suma/mecze/liczba)
            col_s = 'SUMA'
            ...
            # 3. UZUPEŁNIANIE DANYCH (Mecze i Minuty)
            # Mapujemy minuty
            ...
            # [KLUCZOWE] Bierzemy WIĘKSZĄ wartość: albo to co wpisane ręcznie, albo historię występów
            df_uv[col_s] = df_uv[[col_s, 'Mecze_Calc']].max(axis=1)

            # 4. SORTOWANIE WSTĘPNE I USUWANIE DUPLIKATÓW
            # Sortujemy tak, aby w razie duplikatów nazwiska został ten z lepszymi statystykami
            df_uv = df_uv.sort_values([col_s, 'Minuty'], ascending=[False, False]).drop_duplicates(
                subset=['imię i nazwisko'])

            # 5. FILTRY I SORTOWANIE INTERFEJSU
            ...
            if obcy and 'narodowość' in df_uv.columns:
                df_uv = df_uv[~df_uv['narodowość'].astype(str).str.contains("Polska", na=False, case=False)]
            ...
            # 7. PROFIL ZAWODNIKA (Podgląd szczegółowy)
    ...
    with tab2:
        ...
        if df_s is not None:
            ...
            # Filtry
            if sezs:
                df_v = df_v[df_v['sezon'].isin(sezs)]
            if search_s:
                df_v = df_v[df_v['imię i nazwisko'].astype(str).str.contains(search_s, case=False)]

            # Grupowanie (Suma goli dla zawodnika)
            if 'gole' in df_v.columns:
                ...
                st.dataframe(
                    grp[['imię i nazwisko', 'Flaga', 'Narodowość', 'gole']],
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "Flaga": st.column_config.ImageColumn("Flaga", width="small"),
