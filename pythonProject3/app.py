...
def render_player_profile(player_name):
    ...
    # POPRAWA SORTOWANIA (KeyError fix)
    ...
    # --- NOWOŚĆ: ODZNAKI / REKORDY (ULEPSZONY WYGLĄD) ---
    badges = get_player_record_badges(player_name)

    if badges:
        st.write("")  # Odstęp
        # Generowanie HTML dla odznak
        badges_html = ""
        for b in badges:
            badges_html += f"""
                <span style="
...
def load_details(filename="wystepy.csv"):
    ...
    try:
        ...
        # --- NOWA LOGIKA: OBLICZANIE MINUT I KOREKTA CZERWONYCH ---
        ...
        def process_match_stats(row):
            """
            Oblicza:
            1. Rzeczywiste minuty gry (min_played)
            2. Minutę zdarzenia do parowania zmian (event_min)
            3. Poprawną liczbę czerwonych kartek (red_card)
            """
            status = str(row.get('Status', '')).strip()
            val_csv = row['Czas_Zdarzenia']  # Wartość z CSV (np. 80 - minuta wejścia)

            # A. Logika Minut
            min_played = 0
            event_min = 0  # To służy do sortowania zmian (minuta zegarowa)

            if status == 'Cały mecz':
                min_played = 90
                event_min = 0
            elif status == 'Zszedł':
                min_played = val_csv  # Grał tyle, w której minucie zszedł
                event_min = val_csv
            elif status == 'Wszedł':
                # FIX 1: Wejście w 90 minucie daje 1 minutę gry, a nie 0
                calc = 90 - val_csv
            ...
            # B. Logika Czerwonych Kartek
            # Pobieramy to co jest w CSV, ale naprawiamy błędy logiczne
            current_red = int(pd.to_numeric(row.get('Czerwone', 0), errors='coerce') or 0)

            # FIX 2: Jeśli status to 'Zszedł', to NIE MA czerwonej kartki (to zwykła zmiana)
            if status == 'Zszedł':
                current_red = 0

            # Jeśli status to explicite 'Czerwona kartka', musi być 1
            if status == 'Czerwona kartka':
                current_red = 1

            return pd.Series([min_played, event_min, current_red])

        # Aplikujemy nową logikę do DataFrame
...
def extract_scorers_list(scorers_str):
    ...
    for part in parts:
        ...
        # Czysta nazwa do szukania profilu (usuwamy nawiasy i cyfry)
        clean_name = re.sub(r'\(.*?\)', '', part)
        clean_name = re.sub(r'\d+', '', clean_name).strip()
        clean_name = clean_name.replace("s.", "").replace("k.", "").strip()

        display_text = f"{icon} {part}"
        result.append({'display': display_text, 'link_name': clean_name, 'is_own': is_own})
...
def get_minutes_map(scorers_str):
    ...
    for part in parts:
        ...
        # Teraz zadziała, bo korzysta z globalnego 'import re' z góry pliku
    ...
    return mapping
    # --- USUŃ WSZYSTKO PONIŻEJ TEJ LINI W TEJ FUNKCJI ---
    # Dzielimy po przecinkach... (tu był stary kod i import re)
...
def get_player_record_badges(player_name):
    ...
    try:
        ...
        # Musimy to policzyć tutaj, bo pilkarze.csv może mieć starą wartość, a wystepy.csv nowszą
        manual_matches = 0
        if not df_p.empty:
            row = df_p[df_p['imię i nazwisko'] == player_name]
            if not row.empty:
                # Szukamy kolumny suma/mecze/liczba
        ...
        if total_matches >= 100:
            badges.append({"icon": "💯", "text": f"Klub 100 ({total_matches} spotkań)",
                           "color": "#d63031"})  # Czerwony/Prestiżowy
        ...
        if 'Data_Sort' in df_w.columns:
            ...
            # Odsiewamy błędy (wiek > 60 lat)
            df_merged_old = df_merged_old[df_merged_old['Age_Days'] < 22000]
...
# =========================================================
    ...
    if df_det is None or df_bio is None:
        ...
        season_data = df_det[df_det['Sezon'] == sel_season]

        # Agregacja statystyk z występów
        ...
        # --- NOWOŚĆ: NORMALIZACJA POZYCJI (Aby uniknąć "Nieznana pozycja") ---
        def normalize_position_group(val):
            s = str(val).lower().strip()
            # Logika słów kluczowych
            ...
            return "Inne"  # Jeśli kompletnie brak danych
        ...
        def calc_season_age(dob_val):
            ...
            try:
                return season_start_year - pd.to_datetime(dob_val).year
            except:
                return None
        ...
        # Wybór kolumn
        cols_show = ['Flaga', 'Zawodnik_Clean', 'Grupa_Pozycji', 'pozycja', 'Wiek_w_Sezonie', 'Mecze', 'Minuty', 'Gole',
                     'Żółte', 'Czerwone']
        ...
        common_config = {
            ...
            "pozycja": st.column_config.TextColumn("Dokładna Pozycja")  # Opcjonalnie, żeby widzieć co jest w bazie
        }

        # 1. Pętla po głównych formacjach (korzystamy z Grupa_Pozycji)
        for group_key, header_txt in formations_order:
            # Filtrujemy po ZNORMALIZOWANEJ grupie
            df_pos = final_df[final_df['Grupa_Pozycji'] == group_key].sort_values(['Minuty', 'Mecze'], ascending=False)

            if not df_pos.empty:
                st.markdown(f"#### {header_txt}")
                # Ukrywamy kolumnę techniczną Grupa_Pozycji, pokazujemy resztę
                cols_to_display = [c for c in final_df.columns if c != 'Grupa_Pozycji']

                st.dataframe(
                    df_pos[cols_to_display],
                    use_container_width=True,
                    hide_index=True,
                    column_config=common_config
                )

        # 2. Obsługa zawodników, których system nie rozpoznał (Inne)
        df_other = final_df[final_df['Grupa_Pozycji'] == "Inne"].sort_values(['Minuty', 'Mecze'], ascending=False)

        if not df_other.empty:
            st.markdown("#### ❓ Nieznana pozycja (Brak danych w pilkarze.csv)")
            cols_to_display = [c for c in final_df.columns if c != 'Grupa_Pozycji']
            st.dataframe(
                df_other[cols_to_display],
                use_container_width=True,
                hide_index=True,
                column_config=common_config
            )
        ...
        prev_season_str = None
        if curr_idx + 1 < len(seasons):
            prev_season_str = seasons[curr_idx + 1]
    ...
    # Zmieniono liczbę zakładek na 3
    tab1, tab2, tab3 = st.tabs(["Baza Zawodników", "Strzelcy", "Klub 100"])

    # =========================================================
    # TAB 1: BAZA ZAWODNIKÓW (PEŁNA LISTA + PROFIL + ODZNAKI)
    # =========================================================
    with tab1:
        ...
        if df_uv is not None:
            # Słowniki pomocnicze z wystepy.csv (jeśli istnieje)
            minutes_dict = {}
            matches_real_dict = {}

            # PRZYGOTOWANIE DANYCH DO PANELU ODZNAK (AGREGACJA)
            ...
            if df_det is not None:
                if 'Zawodnik_Clean' in df_det.columns:
                    # Agregacja statystyk dla odznak (Szybki skan całej bazy)
                    ...
                    df_det['Gole_Num'] = pd.to_numeric(df_det['Gole'], errors='coerce').fillna(0).astype(int)
                    df_det['Czerwone_Num'] = pd.to_numeric(df_det['Czerwone'], errors='coerce').fillna(0).astype(int)

                    stats_agg = df_det.groupby('Zawodnik_Clean').agg(
                        Max_Gole=('Gole_Num', 'max'),
                        Sum_Gole_Bench=('Gole_Num', lambda x: x[df_det.loc[x.index, 'Status'] == 'Wszedł'].sum()),
                        Sum_Red=('Czerwone_Num', 'sum'),
                        Sezony_Cnt=('Sezon', 'nunique')
                    ).to_dict('index')

                    if 'Data_Sort' in df_det.columns:
                        df_p_temp = df_uv[['imię i nazwisko', 'data urodzenia']].copy()
                        df_p_temp['join_key'] = df_p_temp['imię i nazwisko'].astype(str).str.strip()

                        first_games = df_det.groupby('Zawodnik_Clean')['Data_Sort'].min().reset_index()
                        merged_y = pd.merge(first_games, df_p_temp, left_on='Zawodnik_Clean', right_on='join_key')


                        def calc_age_simple(r):
                            try:
                                return (r['Data_Sort'] - pd.to_datetime(r['data urodzenia'], dayfirst=True)).days
                            except:
                                return 99999
                        ...
                        merged_y['Age_Days'] = merged_y.apply(calc_age_simple, axis=1)
                        top_young_set = set(merged_y.sort_values('Age_Days').head(10)['Zawodnik_Clean'])

                        last_games = df_det.groupby('Zawodnik_Clean')['Data_Sort'].max().reset_index()
                        merged_o = pd.merge(last_games, df_p_temp, left_on='Zawodnik_Clean', right_on='join_key')
                        merged_o['Age_Days'] = merged_o.apply(calc_age_simple, axis=1)
                        merged_o = merged_o[merged_o['Age_Days'] < 25000]
                        top_old_set = set(merged_o.sort_values('Age_Days', ascending=False).head(10)['Zawodnik_Clean'])

                    total_goals = df_det.groupby('Zawodnik_Clean')['Gole_Num'].sum().sort_values(ascending=False).head(
                        10)
                    top_scorer_set = set(total_goals[total_goals > 0].index)

            # 2. Scalanie danych (Mecze)
            col_s = 'SUMA'
            if 'SUMA' not in df_uv.columns:
                if 'mecze' in df_uv.columns:
                    col_s = 'mecze'
                elif 'liczba' in df_uv.columns:
                    col_s = 'liczba'

            if col_s in df_uv.columns:
                if isinstance(df_uv[col_s], pd.DataFrame):
                    df_uv[col_s] = df_uv[col_s].iloc[:, 0]
            ...
            df_uv[col_s] = df_uv[[col_s, 'Mecze_Calc']].max(axis=1)

            df_uv = df_uv.sort_values([col_s, 'Minuty'], ascending=[False, False]).drop_duplicates(
                subset=['imię i nazwisko'])
            df_uv = prepare_flags(df_uv)

            # --- PANEL ODZNAK (Z LEGENDA) ---
            with st.expander("🎖️ Panel Odznak (Skaner i Legenda)"):
                st.info("Szybki przegląd zawodników, którzy zdobyli unikalne odznaki.")

                badge_rows = []
                for _, row in df_uv.iterrows():
                    p_name = row['imię i nazwisko']
                    p_badges = []

                    if row[col_s] >= 100: p_badges.append("💯 Klub 100")
                    if row['Minuty'] > 5000: p_badges.append("🫁 Żelazne Płuca")

                    if p_name in stats_agg:
                        s = stats_agg[p_name]
                        if s['Max_Gole'] >= 3: p_badges.append("🎩 Hat-trick Hero")
                        if s['Sum_Gole_Bench'] >= 3: p_badges.append("🃏 Super Joker")
                        if s['Sum_Red'] >= 2: p_badges.append("🟥 Bad Boy")
                        if s['Sezony_Cnt'] >= 5:
                            p_badges.append("🦅 Weteran")
                        elif s['Sezony_Cnt'] >= 3:
                            p_badges.append("🛡️ Solidny Ligowiec")

                    if p_name in top_young_set: p_badges.append("👶 Debiutant (Top 10)")
                    if p_name in top_old_set: p_badges.append("👴 Nestor (Top 10)")
                    if p_name in top_scorer_set: p_badges.append("👑 Top Strzelec")
                    ...
                    if p_badges:
                        badge_rows.append({
                            ...
                            "Zawodnik": p_name,
                            "Odznaki": p_badges,
                            "Odznaki_Txt": ", ".join(p_badges),
                            "Liczba": len(p_badges)
                        })

                if badge_rows:
                    df_b = pd.DataFrame(badge_rows)
                    all_possible_badges = sorted(list(set([b for sublist in df_b['Odznaki'] for b in sublist])))
                    sel_badges = st.multiselect("Filtruj po odznace:", all_possible_badges)

                    if sel_badges:
                        df_b = df_b[df_b['Odznaki'].apply(lambda x: all(b in x for b in sel_badges))]

                    if not df_b.empty:
                        df_b = df_b.sort_values('Liczba', ascending=False)
                        st.dataframe(
                            df_b[['Flaga', 'Zawodnik', 'Odznaki_Txt', 'Liczba']],
                            use_container_width=True,
                            hide_index=True,
                            column_config={
                                "Flaga": st.column_config.ImageColumn("", width="small"),
                                "Odznaki_Txt": st.column_config.TextColumn("Zdobyte Odznaki"),
                                "Liczba": st.column_config.NumberColumn("Ilość")
                            }
                        )
                    else:
                ...
                # --- LEGENDA ---
                st.divider()
                st.markdown("### ℹ️ Legenda Odznak")
                st.markdown("""
                | Odznaka | Opis / Kryteria |
                | :--- | :--- |
                | 💯 **Klub 100** | Zawodnik rozegrał **100 lub więcej** oficjalnych meczów dla TSP. |
                | 👑 **Top Strzelec** | Zawodnik znajduje się w **TOP 10** najlepszych strzelców w historii klubu. |
                | 🎩 **Hat-trick Hero** | Strzelił **3 gole (lub więcej)** w jednym meczu. |
                | 🃏 **Super Joker** | Zdobył łącznie co najmniej **3 gole** wchodząc z ławki rezerwowych. |
                | 🫁 **Żelazne Płuca** | Spędził na boisku łącznie ponad **5000 minut**. |
                | 🦅 **Weteran** | Zagrał w barwach klubu w co najmniej **5 różnych sezonach**. |
                | 🛡️ **Solidny Ligowiec** | Zagrał w co najmniej **3 różnych sezonach**. |
                | 🟥 **Bad Boy** | Otrzymał w swojej karierze co najmniej **2 czerwone kartki**. |
                | 👶 **Debiutant (Top 10)** | Znajduje się na liście 10 **najmłodszych** debiutantów w historii. |
                | 👴 **Nestor (Top 10)** | Znajduje się na liście 10 **najstarszych** zawodników w historii. |
                """)

            st.divider()

            # 5. FILTRY I SORTOWANIE GŁÓWNE
            c1, c2, c3 = st.columns([2, 1, 1])
            with c1:
                search = st.text_input("Szukaj zawodnika:", key="search_box_players")
            with c2:
                sort_option = st.selectbox("Sortuj wg:", ["Mecze (Najwięcej)", "Mecze (Najmniej)", "Minuty (Najwięcej)",
                                                          "Nazwisko (A-Z)"], index=0)
            with c3:
                st.write("");
                st.write("")
            ...
            if obcy and 'narodowość' in df_uv.columns: df_uv = df_uv[
                ~df_uv['narodowość'].astype(str).str.contains("Polska", na=False, case=False)]
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    col_s: st.column_config.NumberColumn("Mecze", format="%d 👕",
                                                         help="Liczba meczów (Max z bazy lub historii)"),
    ...
    with tab2:
        ...
        if df_s is not None:
            ...
            if 'gole' in df_v.columns:
                ...
                st.warning("Brak kolumny 'gole' w pliku strzelcy.csv")
        else:
    ...
    with tab3:
        ...
        if df_k100 is not None:
            ...
            if col_s in df_k100.columns:
                if isinstance(df_k100[col_s], pd.DataFrame): df_k100[col_s] = df_k100[col_s].iloc[:, 0]
                ...
                st.dataframe(k100[cols_show], use_container_width=True, hide_index=True,
                             column_config={"Flaga": st.column_config.ImageColumn("Flaga", width="small"),
                                            col_s: st.column_config.NumberColumn("Liczba Meczów", format="%d 👕")})
            else:
                st.warning("Brak kolumny z liczbą meczów.")
        else:
            st.error("Nie znaleziono pliku pilkarze.csv")
# =========================================================
...
# =========================================================
    ...
    if df_m is not None:
        ...
        # =========================================================
        # ZAKŁADKA 1: ANALIZA RYWALA (ZMODERNIZOWANA)
        # =========================================================
        with tab1:
            ...
            if df_m is not None and 'rywal' in df_m.columns:
                ...
                # --- 1. PRZYGOTOWANIE DANYCH DO RANKINGU ---
                # Agregujemy statystyki dla WSZYSTKICH rywali
                rival_stats = {}
                ...
                # --- 2. INTERFEJS: PODZIAŁ NA RANKINGI I SZCZEGÓŁY ---
                ...
                if mode == "🏆 Rankingi H2H (Bilans)":
                    ...
                    # Filtr minimalnej liczby meczów (żeby odsiać jednorazowe mecze)
                    min_matches = st.slider("Minimalna liczba rozegranych meczów:", 1, 10, 2)
                    df_rank = df_h2h[df_h2h['Mecze'] >= min_matches].copy()

                    col_best, col_worst = st.columns(2)

                    # A. ULUBIENI RYWALE (Sortowanie: Śr. Pkt -> Bilans -> Mecze)
                    df_best = df_rank.sort_values(['Śr. Pkt', 'Bilans', 'Mecze'], ascending=[False, False, False]).head(
                        10)

                    with col_best:
                        st.markdown("### 🟢 Ulubieni Rywale")
                        st.caption("Najwyższa średnia punktów")
                        st.dataframe(
                            df_best[['Rywal', 'Mecze', 'W', 'R', 'P', 'Bilans', 'Śr. Pkt']],
                            hide_index=True,
                            use_container_width=True,
                            column_config={
                                "Śr. Pkt": st.column_config.ProgressColumn("Śr. Pkt", format="%.2f", min_value=0,
                                                                           max_value=3),
                                "Bilans": st.column_config.NumberColumn("Bilans", format="%+d")
                            }
                        )

                    # B. NIEWYGODNI RYWALE (Sortowanie: Śr. Pkt (rosnąco) -> Bilans (rosnąco))
                    df_worst = df_rank.sort_values(['Śr. Pkt', 'Bilans'], ascending=[True, True]).head(10)

                    with col_worst:
                        st.markdown("### 🔴 Niewygodni Rywale")
                        st.caption("Najniższa średnia punktów")
                        st.dataframe(
                            df_worst[['Rywal', 'Mecze', 'W', 'R', 'P', 'Bilans', 'Śr. Pkt']],
                            hide_index=True,
                            use_container_width=True,
                            column_config={
                                "Śr. Pkt": st.column_config.ProgressColumn("Śr. Pkt", format="%.2f", min_value=0,
                                                                           max_value=3,
                                                                           help="Im mniej zielonego paska, tym gorzej nam szło"),
                                "Bilans": st.column_config.NumberColumn("Bilans", format="%+d")
                            }
                        )

                    st.info(f"💡 Statystyki obliczone dla rywali, z którymi rozegrano minimum {min_matches} spotkań.")

                else:
                    # --- WIDOK SZCZEGÓŁOWY (TO CO BYŁO WCZEŚNIEJ, ALE ULEPSZONE) ---
                    rivals_list = sorted(df_m['rywal'].dropna().unique())
                    selected_rival = st.selectbox("Wybierz przeciwnika:", rivals_list)

                    if selected_rival:
                        # Pobieramy dane z naszego przeliczonego df_h2h dla podsumowania
                        ...
                        st.dataframe(
                            ...
                            hide_index=True,
                            column_config={
                                "dom": st.column_config.TextColumn("Miejsce", help="1 = Dom, 0 = Wyjazd"),
                                "widzów": st.column_config.NumberColumn("Kibiców")
                            }
                        )
            else:
    ...
    # --- FUNKCJA POMOCNICZA: DODAWANIE MEDALI I NUMERACJI ---
    ...
    if df_p is None or df_w is None:
        ...
        # PRZYGOTOWANIE DANYCH (Suma meczów z bazy + historii)
        col_s = 'SUMA'
        if 'SUMA' not in df_p.columns:
            if 'mecze' in df_p.columns: col_s = 'mecze'
            elif 'liczba' in df_p.columns: col_s = 'liczba'
        ...
        if col_s in df_p.columns:
            ...
            matches_real_dict = df_w['Zawodnik_Clean'].value_counts().to_dict() if 'Zawodnik_Clean' in df_w.columns else {}
        ...
        # Unikalna lista piłkarzy z flagami
        df_p_uniq = df_p.sort_values('Total_Matches', ascending=False).drop_duplicates(subset=['imię i nazwisko'], keep='first')
        df_p_uniq = prepare_flags(df_p_uniq)

        # PRZYGOTOWANIE DANYCH STRZELCÓW (SUMA GOLI ALL-TIME)
        top_scorers = pd.DataFrame()
        if df_s is not None:
             top_scorers = df_s.groupby(['imię i nazwisko', 'kraj'], as_index=False)['gole'].sum()
             top_scorers = top_scorers.sort_values('gole', ascending=False).head(10) # TOP 10
             top_scorers = prepare_flags(top_scorers, 'kraj')
             top_scorers = add_rank_medals(top_scorers)

        # --- TABS ---
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["👤 Rekordy Piłkarskie", "🏟️ Rekordy Meczowe", "📅 Rekordy Sezonowe", "📊 Statystyki Drużynowe", "🟥 Kartki i Kary"])

        # ==========================================
        # TAB 1: PIŁKARZE (WIEK, WYSTĘPY, GOLE)
        # ==========================================
        with tab1:
            st.subheader("Legendy Indywidualne")
            
            # SEKJA 1: NAJWIĘCEJ WYSTĘPÓW I GOLI
            col_apps, col_goals = st.columns(2)
            
            with col_apps:
                st.markdown("#### 👕 Najwięcej Występów (TOP 10)")
                top_apps = df_p_uniq.head(10)
                top_apps = add_rank_medals(top_apps)
                
                if not top_apps.empty:
                    st.dataframe(
                        top_apps[['Miejsce', 'Flaga', 'imię i nazwisko', 'Total_Matches']], 
                        hide_index=True, 
                        use_container_width=True,
                        column_config={
                            "Miejsce": st.column_config.TextColumn("Lp."),
                            "Flaga": st.column_config.ImageColumn("", width="small"),
            ...
            with col_goals:
                st.markdown("#### ⚽ Najlepsi Strzelcy (TOP 10)")
                if not top_scorers.empty:
                    st.dataframe(
                        top_scorers[['Miejsce', 'Flaga', 'imię i nazwisko', 'gole']], 
                        hide_index=True, 
                        use_container_width=True,
                        column_config={
                            "Miejsce": st.column_config.TextColumn("Lp."),
                    ...
                    st.info("Brak danych w strzelcy.csv")
            ...
            if 'Data_Sort' in df_w.columns:
                ...
                # Merge występy z datami urodzenia
                ...
                merged_age['Age_Days'] = merged_age.apply(lambda x: calc_age_days(x, 'Data_Sort', 'data urodzenia'), axis=1)
                merged_age['Wiek_Txt'] = merged_age['Age_Days'].apply(lambda x: f"{x//365} lat, {x%365} dni" if 3650 < x < 25000 else "-")
                ...
                top_young = debuts[debuts['Age_Days'] > 3650].head(10)
                ...
                top_old = veterans[veterans['Age_Days'] < 25000].head(10)
                top_old = add_rank_medals(top_old)

                with c_young:
                    st.markdown("#### 👶 Najmłodsi Debiutanci")
                    st.dataframe(top_young[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt', 'Data_Sort']], hide_index=True, use_container_width=True,
                                 column_config={"Miejsce": st.column_config.TextColumn("Lp."), "Flaga": st.column_config.ImageColumn("", width="small"), "Data_Sort": st.column_config.DateColumn("Data")})

                with c_old:
                    st.markdown("#### 👴 Najstarsi Zawodnicy")
                    st.dataframe(top_old[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt', 'Data_Sort']], hide_index=True, use_container_width=True,
                                 column_config={"Miejsce": st.column_config.TextColumn("Lp."), "Flaga": st.column_config.ImageColumn("", width="small"), "Data_Sort": st.column_config.DateColumn("Data")})

                st.divider()

                # --- B. STRZELCY WIEKOWI ---
                st.markdown("### ⚽ Rekordy Wiekowe Strzelców")
                merged_goals = merged_age[pd.to_numeric(merged_age['Gole'], errors='coerce').fillna(0) > 0].copy()
                
                if not merged_goals.empty:
                    c_y_goal, c_o_goal = st.columns(2)
                    
                    # Najmłodszy strzelec
                    youngest_goals = merged_goals.loc[merged_goals.groupby('Zawodnik_Clean')['Age_Days'].idxmin()]
                    top_y_scorers = youngest_goals[youngest_goals['Age_Days'] > 3650].sort_values('Age_Days').head(10)
                    top_y_scorers = add_rank_medals(top_y_scorers)

                    # Najstarszy strzelec
                    oldest_goals = merged_goals.loc[merged_goals.groupby('Zawodnik_Clean')['Age_Days'].idxmax()]
                    top_o_scorers = oldest_goals[oldest_goals['Age_Days'] < 25000].sort_values('Age_Days', ascending=False).head(10)
                    top_o_scorers = add_rank_medals(top_o_scorers)

                    with c_y_goal:
                        st.markdown("#### 👶 Najmłodsi Strzelcy")
                        st.dataframe(top_y_scorers[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt', 'Data_Sort', 'Przeciwnik']], 
                                     hide_index=True, use_container_width=True,
                                     column_config={"Miejsce": st.column_config.TextColumn("Lp."), "Flaga": st.column_config.ImageColumn("", width="small"), "Data_Sort": st.column_config.DateColumn("Data")})
                    
                    with c_o_goal:
                        st.markdown("#### 👴 Najstarsi Strzelcy")
                        st.dataframe(top_o_scorers[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Wiek_Txt', 'Data_Sort', 'Przeciwnik']], 
                                     hide_index=True, use_container_width=True,
                                     column_config={"Miejsce": st.column_config.TextColumn("Lp."), "Flaga": st.column_config.ImageColumn("", width="small"), "Data_Sort": st.column_config.DateColumn("Data")})
                else:
                    st.info("Brak danych o golach połączonych z datami urodzenia.")
            else:
                st.info("Brak dat w pliku występów - nie można obliczyć wieku.")
        ...
        with tab2:
            ...
            if df_m is not None and 'wynik' in df_m.columns:
                def calc_goal_diff(res_str):
                    ...
                    if parsed: return parsed[0] - parsed[1], parsed[0], parsed[1]
                    return -99, 0, 0

                df_m['Diff_Stats'] = df_m['wynik'].apply(calc_goal_diff)
                df_m['Różnica'] = df_m['Diff_Stats'].apply(lambda x: x[0])
                ...
                with c_win:
                    ...
                    cols_show = ['Miejsce', 'data meczu', 'rywal', 'wynik', 'rozgrywki']
                    cols_final = [c for c in cols_show if c in top_wins.columns]
                    st.dataframe(top_wins[cols_final], hide_index=True, use_container_width=True, column_config={"Miejsce": st.column_config.TextColumn("Lp.")})

            # B. NAJWIĘCEJ GOLI W MECZU (ZAWODNIK)
            with c_high:
                ...
                if 'Gole' in df_w.columns:
                    ...
                    best_perfs = df_w[df_w['Gole_Num'] >= 3].sort_values(['Gole_Num', 'Data_Sort'], ascending=[False, False]).head(10)
                    
                    if not best_perfs.empty:
                        best_perfs['Info'] = best_perfs['Przeciwnik'] + " (" + best_perfs['Wynik'] + ")"
                        # Ranking tutaj nie ma sensu numerycznego (dużo remisów), ale lista jest OK
            ...
            # C. REKORDY FREKWENCJI (TYLKO DOMOWE)
            st.divider()
            st.subheader("📢 Rekordy Frekwencji (Dom)")
            ...
            if col_att:
                ...
                # 2. FILTRACJA: TYLKO MECZE DOMOWE (dom == 1)
                if 'dom' in df_m.columns:
                    # Używamy pd.to_numeric dla pewności, że '1' (str) i 1 (int) zadziałają tak samo
                    df_home_only = df_m[pd.to_numeric(df_m['dom'], errors='coerce') == 1].copy()
                else:
                    df_home_only = df_m.copy() # Fallback jeśli brak kolumny 'dom'
                
                # 3. Sortowanie i Ranking
                top_att = df_home_only.sort_values('Att_Num', ascending=False).head(10)
                top_att = top_att[top_att['Att_Num'] > 0] # Usuwamy puste/zera
                top_att = add_rank_medals(top_att)
                
                if not top_att.empty:
                    # Fix JSON error: int()
                    max_att = int(top_att['Att_Num'].max())
                    
                    cols_att_show = ['Miejsce', 'data meczu', 'rywal', 'wynik', 'Att_Num']
                    cols_att_final = [c for c in cols_att_show if c in df_m.columns]
                    
                    st.dataframe(
                        top_att[cols_att_final], 
                        hide_index=True, 
                        use_container_width=True,
                        column_config={
                            "Miejsce": st.column_config.TextColumn("Lp."),
                            "Att_Num": st.column_config.ProgressColumn(
                                "Widzów", 
                                format="%d", 
                                min_value=0, 
                                max_value=max_att
                    ...
                    st.caption("Brak danych o frekwencji dla meczów domowych.")
            else:
                 st.info("Brak kolumny 'widzów' lub 'frekwencja' w pliku mecze.csv")
        ...
        with tab3:
            ...
            with c_king:
                ...
                if df_s is not None:
                    ...
                    st.dataframe(kings[['sezon', 'Flaga', 'imię i nazwisko', 'gole']], hide_index=True, use_container_width=True,
                                 column_config={"Flaga": st.column_config.ImageColumn("", width="small"), "gole": st.column_config.NumberColumn("Gole", format="%d ⚽")})
            
            with c_stat:
                st.subheader("📈 Najlepsze Sezony (Punkty)")
                if df_m is not None and 'wynik' in df_m.columns and 'sezon' in df_m.columns:
                    season_stats = []
                    for season, group in df_m.groupby('sezon'):
                        pts = 0; matches_count = 0; goals_for = 0
                        for _, row in group.iterrows():
                            res = parse_result(row['wynik'])
                            if res:
                                matches_count += 1; goals_for += res[0]
                                if res[0] > res[1]: pts += 3
                                elif res[0] == res[1]: pts += 1
                        if matches_count > 0:
                            ppg = pts / matches_count
                            season_stats.append({'Sezon': season, 'Pkt': pts, 'Mecze': matches_count, 'Gole': goals_for, 'Śr. Pkt': ppg})
                    
                    df_season_stats = pd.DataFrame(season_stats).sort_values('Śr. Pkt', ascending=False)
                    st.dataframe(df_season_stats, hide_index=True, use_container_width=True,
                                 column_config={
                                     "Śr. Pkt": st.column_config.ProgressColumn("Średnia Pkt", format="%.2f", min_value=0, max_value=3),
                                     "Pkt": st.column_config.NumberColumn("Punkty Total"),
                                     "Gole": st.column_config.NumberColumn("Gole Strzelone", format="%d ⚽")
                                 })

        # ==========================================
        # TAB 4: STATYSTYKI DRUŻYNOWE
        # ==========================================
        with tab4:
            st.subheader("📊 Podsumowanie Historyczne")
            if df_m is not None:
                total_m = len(df_m)
                ...
                c_sum1.metric("Wszystkie Mecze", total_m)
                c_sum2.metric("Zwycięstwa", wins)
                c_sum3.metric("Bramki Strzelone", gf)
                c_sum4.metric("Bramki Stracone", ga)
                st.divider()
                if total_m > 0:
                    win_pct = (wins / total_m) * 100
                    st.progress(win_pct / 100, text=f"Skuteczność zwycięstw: {win_pct:.1f}%")
            else:
                st.warning("Brak danych meczowych.")

        # ==========================================
        # TAB 5: KARTKI I KARY (BRUTALE)
        # ==========================================
        with tab5:
            ...
            if df_w is not None:
                ...
                cards_agg = df_w.groupby('Zawodnik_Clean').agg({'Żółte': 'sum', 'Czerwone': 'sum', 'Mecz_Label': 'count'}).reset_index()
                ...
                with col_y_rank:
                    st.markdown("#### 🟨 Najwięcej Żółtych Kartek (TOP 10)")
                    top_yellow = cards_agg.sort_values('Żółte', ascending=False).head(10)
                    top_yellow = add_rank_medals(top_yellow)
                    st.dataframe(
                        top_yellow[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Żółte', 'Mecz_Label']], 
                        hide_index=True, use_container_width=True,
                        column_config={
                            "Miejsce": st.column_config.TextColumn("Lp."),
                            "Flaga": st.column_config.ImageColumn("", width="small"),
                            "Żółte": st.column_config.NumberColumn("Żółte", format="%d 🟨"),
                            "Mecz_Label": st.column_config.NumberColumn("Mecze", help="Liczba meczów")
                        }
                    )
                ...
                with col_r_rank:
                    ...
                    top_red = add_rank_medals(top_red)
                    
                    if not top_red.empty:
                        st.dataframe(
                            top_red[['Miejsce', 'Flaga', 'Zawodnik_Clean', 'Czerwone', 'Mecz_Label']], 
                            hide_index=True, use_container_width=True,
                            column_config={
                                "Miejsce": st.column_config.TextColumn("Lp."),
                                "Flaga": st.column_config.ImageColumn("", width="small"),
